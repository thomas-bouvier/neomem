...
stream_loader_provider_t::stream_loader_provider_t(tl::engine& e,
    ...
    int64_t seed, const std::vector<std::pair<std::string, int>>& endpoints)
        : tl::provider<stream_loader_provider_t>(e, server_id), K(_K), N(_N),
        C(_C), rand_gen(seed),
        async_thread(&stream_loader_provider_t::async_process, this) {
    define("get_samples", &stream_loader_provider_t::get_remote_samples);
...
void stream_loader_provider_t::async_process() {
    while (true) {
...
void stream_loader_provider_t::get_remote_samples(const tl::request& req, unsigned int index) {
    get_samples(index);
    req.respond(selected_samples);
}

// TODO: pass a vector of indices
void stream_loader_provider_t::get_samples(unsigned int index) {
    if (rehearsal_size == 0)
        return;
...
void stream_loader_provider_t::accumulate(const torch::Tensor &samples, const torch::Tensor &labels,
...
int stream_loader_provider_t::wait() {
    std::unique_lock<std::mutex> lock(request_mutex);
...
size_t stream_loader_provider_t::get_rehearsal_size() {
    return rehearsal_size;
}

size_t stream_loader_provider_t::get_history_count() {
    return history_count;
}

stream_loader_provider_t::~stream_loader_provider_t() {
    std::unique_lock<std::mutex> lock(request_mutex);
