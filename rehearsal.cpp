...
PYBIND11_MODULE(rehearsal, m) {
    ...
    py::class_<distributed_stream_loader_recon_t>(m, "DistributedStreamLoaderRecon")
        .def(py::init<unsigned int, unsigned int, int64_t, uint16_t, std::string, std::vector<std::pair<int, std::string>>&>(), py::call_guard<py::scoped_ostream_redirect>())
        .def("accumulate", &distributed_stream_loader_recon_t::accumulate, py::call_guard<py::gil_scoped_release>())
        .def("wait", &distributed_stream_loader_recon_t::wait, py::call_guard<py::gil_scoped_release>())
        .def("get_rehearsal_size", &distributed_stream_loader_recon_t::get_rehearsal_size)
        .def("get_history_count", &distributed_stream_loader_recon_t::get_history_count);
}
