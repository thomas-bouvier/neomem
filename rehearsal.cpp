...
PYBIND11_MODULE(rehearsal, m) {
    ...
    */
    ...
    py::class_<distributed_stream_loader_t>(m, "DistributedStreamLoader")
        .def(py::init<Task, unsigned int, unsigned int, unsigned int, int64_t, uint16_t, std::string, std::vector<std::pair<int, std::string>>&, unsigned int, std::vector<long>>(), py::call_guard<py::scoped_ostream_redirect>())
