...
struct rpc_response_t {
    ...
    rpc_response_t(RPCResponseType type, size_t num_elements, size_t offset, long int label, float weight) : m_type(type), m_num_elements(num_elements), m_offset(offset), m_label(label), m_weight(weight) { }
