...
INCLUDES=$(shell python -m pybind11 --includes) $(shell pkg-config --cflags-only-I thallium) $(TORCH_INCLUDE)
LIBS=-L$(TORCH_ROOT)/lib -ltorch $(shell pkg-config --libs thallium)
